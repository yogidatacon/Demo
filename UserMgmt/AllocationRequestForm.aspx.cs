...
namespace UserMgmt
{
    public partial class AllocationRequestForm : System.Web.UI.Page
    {
                        ...
                        if (user.role_name == "Applicant" || user.party_type == "ALL" || user.party_type == "All")
                        {
                            ...
                            if (user.role_name == "Applicant")
                            {
                                var ind = (from s in indents
                                           where s.party_code == user.party_code && s.record_active =="True" && s.record_status == "Y" && s.financial_year==user.financial_year
                                           select s);
                            ...
                            if (Session["rtype"].ToString() != "0")
                            {
                                ...
                                allotment = BL_Molasses_Allocation.GetDetails(Session["Allotment_ID"].ToString(), Session["financial_year"].ToString());
                                ...
                                var ind = (from s in indents
                                           where s.party_code == allotment.party_code && s.record_active =="True" && s.record_status == "Y"
                                           select s);
                                ddlMolassestype.DataSource = ind.ToList();
                                ddlMolassestype.DataTextField = "product_name";
                                ddlMolassestype.DataValueField = "product_code";
                                ...
                                txtQuantityallottedtilldate.Text = allotment.qty_allotted_till_date.ToString();
                                ...
                                if (Session["UserID"].ToString() == "Admin")
                                {
                                    ...
                                    approvals = BL_All_Approvals.GetApprovals(Session["UserID"].ToString(), Session["Allotment_ID"].ToString(), "ALT");
                                ...
                                if ((Session["rtype"].ToString() == "1"))
                                {
                                    ...
                                    workflow = BL_WorkFlow.ApprovelLevels("MA", "110");
                                   ...
                                   else if (aplevel.ToList().Count > 0 || user.role_name == "Applicant")
                                    {
                                        if (user.role_name != "Applicant")
                                        ...
                                        approvals = BL_All_Approvals.GetApprovals(Session["UserID"].ToString(), Session["Allotment_ID"].ToString(), "ALT");
                                        var list = (from s in approvals
                                                    where s.financial_year== Session["financial_year"].ToString()
                                                    select s);
                                        grdApprovalDetails.DataSource = list.ToList();
                                        approv.Visible =true;
                                        grdApprovalDetails.DataBind();
                                        ...
                                        else
                                        {
                                            if ((user.party_type == "All" || user.party_type == "ALL"))
                                            {
                                                ...
                                                if ((allotment.approverlevel == (workal - 1) && allotment.record_status != "N") || (allotment.approverlevel == 1 && allotment.record_status == "B"))
                                                {
        ...
        protected void btnApprove_Click(object sender, EventArgs e)
        {
            if (IsPostBack)
            {
                ...
                allotment.record_status = "Y";
                allotment.party_code = Session["aparty_code"].ToString();
                allotment.user_id = Session["UserID"].ToString();
                int i = 0;
                dt = ViewState["Records"] as DataTable;
                allotment.docs = new List<EASCM_DOCS>();
        ...
        protected void ddlMolassestype_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddlMolassestype.SelectedValue!="Select")
            {
                ...
                indents = BL_IndentForm.GetList();
                var ind = (from s in indents
                           where s.product_code ==ddlMolassestype.SelectedValue && s.record_active =="True" && s.party_code ==partycode.Value && s.financial_year== txtfinancialyear.Text
                           select s);
                var allotedtilldate = from s1 in indents
                                      where s1.product_code == ddlMolassestype.SelectedValue && s1.record_active =="True" && s1.party_code == partycode.Value && s1.financial_year == txtfinancialyear.Text
                                      group s1 by s1.product_code into playerGroup
                                      select new
                                      {
                                          product_code = playerGroup.Key,
                ...
                var list = (from s in partymasters
                            where s.product_code ==ddlMolassestype.SelectedValue && s.party_type_code=="SGR"
                            select s).ToList();
                ...
                txtQuantityallottedtilldate.Text = allotedtilldate.ToList()[0].Totalalloted.ToString();
                CalendarExtender.EndDate = DateTime.Now;
            }
        }
        ...
        protected void btnupdate_Click(object sender, EventArgs e)
        {
            if (IsPostBack)
            {
                ...
                allotment.molasses_allotment_request_id = Session["Allotment_ID"].ToString();
                allotment.allotment_validdate = txtvalieddate1.Value;
                allotment.req_allotmentdate = txtvalieddate1.Value;
                allotment.user_id = Session["UserID"].ToString();
                allotment.financial_year = txtfinancialyear.Text;
                string val;
                val = BL_Molasses_Allocation.adminupdate(allotment);
                if (val == "0")
                {
                    Session["UserID"] = Session["UserID"];
                    Response.Redirect("AllocationRequestList");
    ...
    //public class SugarMils
    //{
    //    public string ProductValue { get; set; }

    //    public List<Party_Master> sugarmills { get; set; }
    //}
