    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT  COUNT(DISTINCT comid) as total_complaints"
                + " FROM   exciseautomation.view_callcentercomplaint_list", cn))
            {
                // fill data table
                da.Fill(dtt);
        ...
        chartConfig.Add("caption", "Call Center Complaints Status"); // caption will change dynamically based on chart label
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT  complaintstatus,COUNT(DISTINCT comid) as counts"
                + " FROM   exciseautomation.view_callcentercomplaint_list group by complaintstatus", cn))
            {
                // fill data table
                da.Fill(dt);
