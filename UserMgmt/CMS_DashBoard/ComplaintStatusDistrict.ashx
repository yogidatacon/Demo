...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT  COUNT(DISTINCT comid) as total_complaints"
                + " FROM   exciseautomation.view_callcentercomplaint_list  where district_code='GA'", cn))
            {
                // fill data table
                da.Fill(dtt);
                total = dtt.Rows[0]["total_complaints"].ToString();
        ...
        chartConfig.Add("caption", "Call Center Complaints Status Gaya District"); // caption will change dynamically based on chart label
        ...
        chartConfig.Add("defaultcenterlabel", "Complaints Received : "+ total);
        ...
        chartConfig.Add("formatNumber", "0");
        chartConfig.Add("startingAngle", "195");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            con.Open();
            using (NpgsqlCommand command = new NpgsqlCommand())
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT  complaintstatus,COUNT(DISTINCT comid) as counts"
                + " FROM   exciseautomation.view_callcentercomplaint_list where district_code='GA' group by district_code,district_name ,complaintstatus order by 1 ", cn))
            {
                // fill data table
                da.Fill(dt);
