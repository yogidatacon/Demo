    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT dm.division_name ,count(seizureno) as SeizureNoCount FROM exciseautomation.seizure_basicinfo sb"
                + " inner join exciseautomation.division_master dm on dm.division_code =sb.division_code where sb.creation_date='" + DateTime.Now.ToString("dd-MM-yyyy") + "' group by sb.division_code,dm.division_name order by dm.division_name", cn))
            {
                // fill data table
                da.Fill(dt);
if (dt.Rows.Count == 0)
