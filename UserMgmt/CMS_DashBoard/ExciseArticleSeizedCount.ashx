...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("formatNumberScale","1");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select acm.article_category_name ,ROUND(sum(quantity::numeric),2) from exciseautomation.seizure_excisable_articles sea"
                                                                + " inner join exciseautomation.article_category_master acm on acm.article_category_code =sea.article_category_code"
                                                                + " group by acm.article_category_code ,acm.article_category_name order by acm.article_category_name", cn))
            {
                // fill data table
                da.Fill(dt);
