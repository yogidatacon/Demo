...
namespace UserMgmt
{
    public partial class DepartmentMasterForm : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (rtype != "0")
                {
                    List<Department> party = new List<Department>();
                    party = BL_User_Mgnt.GetDeptList("");
                    var list = from s1 in party
                               where s1.dept_code == Session["department_code"].ToString()
                               select s1;
                    if (rtype == "1" || list.ToList().Count > 0)
                    {
                        txtcode.Text = Session["department_code"].ToString();
                        txtname1.Text = Session["department_name"].ToString();
                    ...
                    else if (rtype == "2")
                    {
                        txtcode.Text = Session["department_code"].ToString();
                        txtname1.Text = Session["department_name"].ToString();
                        producttypename1 = Session["department_name"].ToString();
                        txtcode.Enabled = false;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (rtype == "0")
            {
                ...
                product.dept_code = txtcode.Text.ToUpper();
                product.dept_name = s;
                product.user_id = user_id;
                if (BL_ProductType.InsertDept(product))
                {
                    Session["UserID"] = Session["UserID"];
                    Response.Redirect("DepartmentMasterList.aspx");
                }
            ...
            else
            {
                ...
                if (BL_ProductType.UpdateDept(product))
                {
        ...
        public static string chkDuplicateProductTypeName(Object producttypename)
        {
            ...
            if (producttypename1 != producttypename.ToString())
            {
                ...
                s = Regex.Replace(s, @"(^\w)|(\s\w)", m => m.Value.ToUpper());
                value = BL_User_Mgnt.GetExistsData("department_master", "department_name", s);
            }
        ...
        public static string chkDuplicateProductTypeCode(Object producttypecode)
        {
            ...
            value = BL_User_Mgnt.GetExistsData("department_master", "department_code", producttypecode.ToString().ToUpper());
