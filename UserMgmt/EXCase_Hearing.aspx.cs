...
namespace UserMgmt
{
    public partial class EXCase_Hearing : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                if (Session["rtype"].ToString() != "0")
                {
                    ...
                    obj = BL_cm_court.GetEXDetails(Session["excase_registration_id"].ToString());
                    ...
                    for (int i = 0; i < obj.docs1.Count; i++)
                    {
                        ...
                        dt1.Rows.Add("", obj.docs1[i].doc_name, obj.docs2[i].document_type, obj.docs1[i].description, obj.docs1[i].doc_path, obj.docs1[i].seizure_docs_id);
        ...
        protected void btnRemove_Click(object sender, EventArgs e)
        {
            if (IsPostBack)
            {
                ...
                {
                    bool value = BL_User_Mgnt.Deletefile("seizure_docs", Session["excase_registration_id"].ToString(), v);
                    if (value)
                    {
        ...
        protected void btnSaveasDraft_Click(object sender, EventArgs e)
        {
            if (IsPostBack)
            {
                if (IsPostBack)
                {
                    ...
                    //else
                    //{
                    cm_obj.excase_registration_id = Convert.ToInt32(Session["excase_registration_id"].ToString());
                    val = BL_cm_court.UpdateEXEntry(cm_obj);
                    //   }
                    if (val == true)
                    {
                        Session["UserID"] = Session["UserID"];
                        Response.Redirect("EXCase_List");
                    }
                    else
                    {
            ...
            if (IsPostBack)
            {
                if (IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        cm_obj.hearing_status = "Disposed at Commissioner Court";
                    }
