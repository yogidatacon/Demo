...
public class Handler : IHttpHandler
{
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select 'Lifted Quantity' as Status,LiftedQuantity from exciseautomation.ViewMolassesLiftedQty2 union select 'Pending Quantity' as Status,PendingQuantity from exciseautomation.ViewMolassesLiftedQty2", cn))
            {
                // fill data table
                da.Fill(dt);
