...
namespace UserMgmt.Handler
{
    ...
    /// </summary>
    public class Allotment : IHttpHandler, IRequiresSessionState
    {
        ...
        private string ProcessChartData(DataTable dt)
        {
            ...
            chartConfig.Add("exportFormats", "PNG=Export as High Quality Image|PDF=Export as Printable|XLSX=Export Chart Data");
            chartConfig.Add("exportFileName", "Molasses Allocation Status");
            ...
            chartConfig.Add("labelFontSize", "9");
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    string link = string.Format(linkParam, row[0].ToString(), "4");
                    data.AppendFormat("{{\"label\":\"{0}\",\"value\":\"{1}\", \"link\": \"{2}\"}},", row[0].ToString(), row[1].ToString(), link);
        ...
        private void GetChartData(ref DataTable dt,string userid)
        {
            ...
            using (NpgsqlConnection con = new NpgsqlConnection(connstring))
            {
                ...
                using (NpgsqlCommand command = new NpgsqlCommand())
                    ...
                    {
                        using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT * FROM exciseautomation.allotmentdashboard", cn))
                        {
                            // fill data table
                            da.Fill(dt);
                ...
                //else if (userid == "hodyco")
                //    {
                //        using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT * FROM exciseautomation.hodallotmentdashboard", cn))
                //        {
                //            // fill data table
                //            da.Fill(dt);
                ...
                //    }
                    ...
                    {
                        using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT caseno,countno FROM exciseautomation.allotmentdashboard1 where user_id='"+userid+"' ", cn))
                        {
                            // fill data table
                            da.Fill(dt);
