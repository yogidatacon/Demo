...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "Seized Apparatus Status"); // caption will change dynamically based on chart label
        chartConfig.Add("xAxisName", "Apparatus Status"); // xaxis name will chnage dynamically based on chart label
        chartConfig.Add("yAxisName", "Total Apparatus Seized");
        ...
        chartConfig.Add("palettecolors", "#ff988e");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT cs.status,count(apparatusid) as ApparatusCount "
                    + " from exciseautomation.casemgmt_seizedapparatusdetails right join exciseautomation.casemgmt_status cs"
                    + " on cs.status=apparatusstatus group by cs.status", cn))
            {
                // fill data table
                da.Fill(dt);
