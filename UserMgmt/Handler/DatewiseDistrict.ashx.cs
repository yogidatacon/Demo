...
{
    /// <summary>
    /// Summary description for DatewiseDistrict
    /// </summary>
    public class DatewiseDistrict : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string[] levelValueMapping = new string[] { "Thana", "Date" };
        ...
        private string ProcessChartData(DataTable dt)
        {
           ...
           // string linkParam = "newchart-jsonurl-Handler/TodayDistrictHandler.ashx?label={0}&drillLevel={1}";
            ...
            chartConfig.Add("xAxisName", "Thana"); // xaxis name will chnage dynamically based on chart label
        ...
        private void GetChartData(ref DataTable dt)
        {
            ...
            using (NpgsqlConnection con = new NpgsqlConnection(connstring))
            {
                ...
                using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select TO_CHAR(createddate :: date, 'dd/mm/yyyy') as createddate, count(seizureno) as SeizureNoCount  from exciseautomation.casemgmt_basicinformation where thana='Katihar SHO' group by  createddate order by count(seizureno) desc", cn))
                {
                    // fill data table
                    da.Fill(dt);
