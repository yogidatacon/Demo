...
{
    /// <summary>
    /// Summary description for MF2_STATUS
    /// </summary>
    public class MF2_STATUS : IHttpHandler
    {
        ...
        private string ProcessChartData(DataTable dt)
        {
            ...
            chartConfig.Add("caption", "MF2 Entry Status "); // caption will change dynamically based on chart label
            chartConfig.Add("xAxisName", "MF2 Entry Status"); // xaxis name will chnage dynamically based on chart label
            chartConfig.Add("yAxisName", "MF2 Entry Status");
        ...
        private void GetChartData(ref DataTable dt)
        {
            ...
            using (NpgsqlConnection con = new NpgsqlConnection(connstring))
            {
                ...
                using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select r.user_name,Count(distinct mf2_producer) as MF2_STATUS_COUNT from exciseautomation.mf2"
                                                                    + " right join exciseautomation.user_registration r on r.user_id=mf2_producer"
                                                                    + " where department_name='Sugar Mill' and role_name='Applicant'"
