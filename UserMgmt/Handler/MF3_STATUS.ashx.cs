...
{
    /// <summary>
    /// Summary description for MF3_STATUS
    /// </summary>
    public class MF3_STATUS : IHttpHandler
    {
        ...
        private string ProcessChartData(DataTable dt)
        {
            ...
            chartConfig.Add("caption", "MF3 Entry Status "); // caption will change dynamically based on chart label
            chartConfig.Add("xAxisName", "MF3 Entry Status"); // xaxis name will chnage dynamically based on chart label
            chartConfig.Add("yAxisName", "MF3 Entry Status");
        ...
        private void GetChartData(ref DataTable dt)
        {
            ...
            using (NpgsqlConnection con = new NpgsqlConnection(connstring))
            {
                ...
                using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select r.user_name,Count(distinct mf3_producer) as MF2_STATUS_COUNT from exciseautomation.mf3"
                                                                    + " right join exciseautomation.user_registration r on r.user_id=mf3_producer"
