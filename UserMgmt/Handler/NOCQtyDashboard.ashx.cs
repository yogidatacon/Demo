...
{
    /// <summary>
    /// Summary description for NOCQtyDashboard
    /// </summary>
    public class NOCQtyDashboard : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Session["UserID"].ToString() != null)
            {
                ...
                party_code = context.Session["party_code"].ToString();
            }
            ...
            GetChartData(ref dt,userid, party_code);
        ...
        private string ProcessChartData(DataTable dt)
        {
            ...
            chartConfig.Add("exportFileName", "NOC Dispatch Status");
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    //}
                    string link = string.Format(linkParam, row[0].ToString(), "18");
                    data.AppendFormat("{{\"label\":\"{0}\",\"value\":\"{1}\", \"link\": \"{2}\"}},", row[0].ToString(), row[1].ToString(), link);
        ...
        private void GetChartData(ref DataTable dt,string userid,string party_code)
        {
            ...
            using (NpgsqlConnection con = new NpgsqlConnection(connstring))
            {
                ...
                using (NpgsqlCommand command = new NpgsqlCommand())
                    ...
                    {
                        using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select 'Total Lifted Quantity' as Status,totalliftedquantity as Quantity from exciseautomation.comnocqtydispatchdashboard  union select 'Total Pending Quantity' as Status,(totalnocquantity - totalliftedquantity) as Quantity from exciseautomation.comnocqtydispatchdashboard", cn))
                        {
                            // fill data table
                            da.Fill(dt);
                        }
                    ...
                    {
                        using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select 'Total Lifted Quantity' as Status,totalliftedquantity as Quantity from exciseautomation.nocqtydispatchdashboard where party_code='"+ party_code + "' union select 'Total Pending Quantity' as Status,(totalnocquantity - totalliftedquantity) as Quantity from exciseautomation.nocqtydispatchdashboard where party_code='"+ party_code +"'", cn))
                        {
                            // fill data table
                            da.Fill(dt);
