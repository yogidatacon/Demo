...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "Seized Property Status"); // caption will change dynamically based on chart label
        chartConfig.Add("xAxisName", "Property Status"); // xaxis name will chnage dynamically based on chart label
        chartConfig.Add("yAxisName", "Total Property Seized");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT cs.status,count(propertyid) as PropertyCount "
                    + " from exciseautomation.casemgmt_seizedpropertydetails right join exciseautomation.casemgmt_status cs"
                    + " on cs.status=propertystatus group by cs.status", cn))
            {
                // fill data table
                da.Fill(dt);
