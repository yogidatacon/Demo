...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "Molasses Release Request Status"); // caption will change dynamically based on chart label
        chartConfig.Add("xAxisName", "Particulars"); // xaxis name will chnage dynamically based on chart label
        chartConfig.Add("yAxisName", "Quantity");

        chartConfig.Add("theme", "fusion");
        chartConfig.Add("defaultcenterlabel", "Status");
        ...
        chartConfig.Add("paletteColors", "#CD5C5C,#FF8C00,#008080");
        ...
        chartConfig.Add("plotFillAngle", "50");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select 'Total Allotted Quantity',sum(distinct allocation_qty) from  exciseautomation.release_request group by allotment_number union all"
                                                                + " select 'Total Release Request Quantity',sum(rr_quantity) from exciseautomation.release_request where approval_status is null or approval_status='Pending' union all"
                                                                + " select 'Total Release Request Approved Quantity', case when sum(rr_quantity) is null then '0' else sum(rr_quantity) end as sum  from exciseautomation.release_request where approval_status='Issued'", cn))
            {
                // fill data table
                da.Fill(dt);
