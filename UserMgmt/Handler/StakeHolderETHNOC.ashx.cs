...
{
    /// <summary>
    /// Summary description for StakeHolderETHNOC
    /// </summary>
    public class StakeHolderETHNOC : IHttpHandler
    {
        ...
        private string ProcessChartData(DataTable dt)
        {
            ...
            chartConfig.Add("exportEnabled", "1");
                   chartConfig.Add("exportFileName", " Molasses Allocation Status At Different Stake Holders");
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    string link = string.Format(linkParam, row[0].ToString(), "102");
                    data.AppendFormat("{{\"label\":\"{0}\",\"value\":\"{1}\", \"link\": \"{2}\"}},", row[0].ToString(), row[1].ToString(), link);
        ...
        private void GetChartData(ref DataTable dt)
        {
            ...
            using (NpgsqlConnection con = new NpgsqlConnection(connstring))
            {
                ...
                using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT * FROM exciseautomation.stakeholderethnon", cn))
                {
                    // fill data table
                    da.Fill(dt);
