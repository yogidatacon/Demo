...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
       ...
       string linkParam = "P-detailsWin,width=1300,height=1000,toolbar=no,scrollbars=yes, resizable=no-DashBoardDetails.aspx?label={0}&viewno={1}";
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            foreach (DataRow row in dt.Rows)
            {
                ...
                string link = string.Format(linkParam,row[0].ToString(), "103");
                data.AppendFormat("{{\"label\":\"{0}\",\"value\":\"{1}\", \"link\": \"{2}\"}},", row[0].ToString(), row[1].ToString(),link);
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select 'Assistant Commissioner' as RoleName,count(noc_number) as RecCount from exciseautomation.noc n  where noc_status ='Created' and noc_for ='Molasses' union select 'Deputy Commissioner' as RoleName,count(noc_number) as RecCount  from exciseautomation.noc n  where noc_status ='Recommended By Assistant Commissioner' and noc_for ='Molasses' union select 'Commissioner' as RoleName,count(noc_number) as RecCount  from exciseautomation.noc n  where noc_status ='Recommended By Deputy Commissioner' and noc_for ='Molasses'", cn))
            {
                // fill data table
                da.Fill(dt);
