...
public class Handler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string[] levelValueMapping = new string[] { "District", "Thana","createddate" };
    ...
    {
        string query;
        // query = "select " + columnName + ", count(seizureno) as SeizureNoCount" + " from exciseautomation.seizure_basicinfo group by  " + columnName;
        query = "select b.thana_name, count(seizureno) as SeizureNoCount from exciseautomation.seizure_basicinfo a inner join exciseautomation.thana_master b on a.thana_code=b.thana_code inner join exciseautomation.district_master c on a.district_code=c.district_code Group by b.thana_name";
        return query;
    }
    private string BuildQuery(string columnName, string parentValue, string parentName)
    {
        string query;
        //query = "select " + columnName + ", count(seizureno) as SeizureNoCount" + " from exciseautomation.seizure_basicinfo where " + parentName + "= '" + parentValue + "' Group by " + columnName;
        query = "select b.thana_name, count(seizureno) as SeizureNoCount from exciseautomation.seizure_basicinfo a inner join exciseautomation.thana_master b on a.thana_code=b.thana_code inner join exciseautomation.district_master c on a.district_code=c.district_code    where c.district_name='" + parentValue + "' Group by b.thana_name";
    ...
    private string ProcessChartData(DataTable dt, string columnName, string drillLevel, int maxLevel)
    {
        ...
        string linkParam = "newchart-jsonurl-Handler/TodayDistrictHandler.ashx?label={0}&drillLevel={1}";
