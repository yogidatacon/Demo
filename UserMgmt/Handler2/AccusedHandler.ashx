...
public class Handler : IHttpHandler
{
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select 'Bail Request',count(*) as TotalBailRequest from exciseautomation.casemgmt_bail where isreleasedonbail in ('','Granted','Rejected') union all"
                                                                + " select 'Bail Granted',count(*) as GrantedCountdata from exciseautomation.casemgmt_bail where isreleasedonbail='Granted' union all"
                                                                + " select 'Bail Rejected',count(*) as RejectedCountdata from exciseautomation.casemgmt_bail where isreleasedonbail='Rejected'  union all"
                                                                + " select 'Arrested',count(*) as TotalArrested from exciseautomation.casemgmt_accuseddetails", cn))
            {
                // fill data table
                da.Fill(dt);
