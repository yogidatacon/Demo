...
public class Handler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        string linkParam = "P-detailsWin,width=1600,height=1000,toolbar=no,scrollbars=yes, resizable=no-DigiLockDetails.aspx?label={0}&drillLevel={1}";

        chartConfig.Add("caption", "DIGILOCKER STATUS - OVERALL "); // caption will change dynamically based on chart label
        ...
        chartConfig.Add("defaultcenterlabel", "Digilocker Status");
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            foreach (DataRow row in dt.Rows)
            {
                ...
                string link = string.Format(linkParam, userid.ToString(), row[0].ToString());
                data.AppendFormat("{{\"label\":\"{0}\",\"value\":\"{1}\", \"link\": \"{2}\"}},", row[0].ToString(), row[2].ToString(), link);
            }
        }
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select Alert_type,Source_district,Sum(Count) as DigiLockCount from exciseautomation.view_digilocker_districtwise"
                + " where source_district ='" + userid.ToString() + "' group by Alert_type,Source_district", cn))
            {
                // fill data table
                da.Fill(dt);
