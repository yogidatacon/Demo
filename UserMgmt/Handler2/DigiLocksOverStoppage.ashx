...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "TOTAL  No.of   VEHICLE's OVER STOPPAGE      ( > 30 MINS )  -  DISTRICT WISE"); // caption will change dynamically based on chart label
        chartConfig.Add("xAxisName", "DISTRICT"); // xaxis name will chnage dynamically based on chart label
        chartConfig.Add("yAxisName", "TOTAL  No.of   VEHICLE's OVER STOPPAGE       ( > 30 MINS )");
        chartConfig.Add("numberSuffix", "");
        chartConfig.Add("theme", "fusion");
        chartConfig.Add("defaultcenterlabel", "TOTAL LOCKS CLOSED" + i);
        ...
        chartConfig.Add("palettecolors", "#f2b556");
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            foreach (DataRow row in dt.Rows)
            {
                ...
                string link = string.Format(linkParam, row[0].ToString(), "3");
                data.AppendFormat("{{\"label\":\"{0}\",\"value\":\"{1}\", \"link\": \"{2}\"}},", row[0].ToString(), row[1].ToString(), link);
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            con.Open();
                ...
                using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select source_district,sum(count) as LockCountss from exciseautomation.View_digilocker_districtwise"
                    ...
                    + " where alert_type='OVER STOPPAGE ( > 30 MINS )' group by source_district ORDER BY sum(count) DESC ", cn))
                {
                    // fill data table
                    da.Fill(dt); 
