...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "Molasses Licence Application Status"); // caption will change dynamically based on chart label
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select 'Licence Request',count(*) as LicenceRequestCountdata from exciseautomation.molasses  union all"
                                                                + " select 'Approved',count(*) as ApprovedCountdata from exciseautomation.molasses where issued='Pending' union all"
                                                                + " select 'Pending',count(*) as ApprovedCountdata from exciseautomation.molasses where issued is null and status <>'Rejected'  union all"
                                                                + " select 'Issued',count(*) as IssuedCountdata from exciseautomation.molasses where issued='Issued'  union all"
                                                                + " select 'Rejected',count(*) as RejectedCountdata from exciseautomation.molasses where status='Rejected'  ", cn))
            {
                // fill data table
                da.Fill(dt);
