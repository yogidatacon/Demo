...
public class Handler : IHttpHandler
{
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT district,count(seizureno) as SeizureNoCount FROM exciseautomation.casemgmt_basicinformation where CreatedDate='" + DateTime.Now.ToString("dd-MM-yyyy") + "' group by district order by count(seizureno) desc", cn))
            {
                // fill data table
                da.Fill(dt);
