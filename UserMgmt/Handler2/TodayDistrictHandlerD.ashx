...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        string linkParam = "newchart-jsonurl-Handler/TodayDistrictHandlerD.ashx?label={0}&drillLevel={1}";
        int i = 0;
        ...
        chartConfig.Add("caption", "Total Seizures by District - Today ( " + i.ToString() + " )"); // caption will change dynamically based on chart label
    ...
    private void GetChartData(ref DataTable dt)
    {
     ...
     //string databaseName = "DrillDownDB";
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT district,count(seizureno) as SeizureNoCount"
                + " FROM exciseautomation.casemgmt_basicinformation where district='" + userid.ToString() + "'"
                + " and CreatedDate='" + DateTime.Now.ToString("dd-MM-yyyy") + "' group by district order by count(seizureno) desc", cn))
            {
                // fill data table
                da.Fill(dt);
