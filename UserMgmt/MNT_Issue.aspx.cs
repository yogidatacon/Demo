...
namespace UserMgmt
{
    public partial class MNT_Issue : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else if (user.role_name == "Bond Officer")
                {
                    ...
                    ddlIssuedVat.Attributes.Add("disabled", "disabled");
                ...
                var ind1 = (from s in vats
                            where s.vat_type_code == "ISS"
                            select s);

                ddlIssuedVat.DataSource = ind1.ToList();
                ddlIssuedVat.DataTextField = "vat_name";
                ddlIssuedVat.DataValueField = "vat_code";
                ddlIssuedVat.DataBind();
                ddlIssuedVat.Items.Insert(0, "Select");
                ...
                if (Session["rtype"].ToString() != "0")
                {
                    ...
                    if (approvals.Count > 0)
                    {
                        var list4 = (from s in approvals
                                     where s.financial_year == Session["isfinancial_year"].ToString()
                                     select s);
                ...
                if (Session["rtype"].ToString() == "1")
                {
                    ...
                    ic = BL_Molasses_Allocation.GetMNTIssueRegDetails(Session["mirid"].ToString(), Session["isfinancial_year"].ToString());
                    ...
                    using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
                    {
                        ...
                        using (NpgsqlCommand cmd1 = new NpgsqlCommand("select iri.issue_register_id, iri.storage_vat as storage_vat_code, iri.issue_vat as issue_vat_code, iri.issue_qty as issued_qty_bl,"
                            + " cast(iri.issue_qty_lpl as text) as issued_qty_lpl, iri.strength,vm.vat_name as issue_vat,vm2.vat_name as storage_vat"
                            + " from exciseautomation.issue_register_item iri  inner join exciseautomation.vat_master vm on vm.vat_code =iri.issue_vat"
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            if (IsPostBack)
            {
                ...
                dummytable.Visible = false;
                string issue_vat = ddlIssuedVat.SelectedItem.ToString();
                ...
                string issue_vat_code = ddlIssuedVat.SelectedValue.ToString();
                ...
                foreach (DataRow row in dt.Rows)
                {
                    if (row["storage_vat_code"].ToString() == ddlStorageVAT.SelectedValue.ToString()
                        & row["issue_vat_code"].ToString() == ddlIssuedVat.SelectedValue.ToString())
                ...
                if (!ifExist)
                {
                    dt.Rows.Add(storage_vat_code, storage_vat, issue_vat_code,issue_vat, issued_qty_bl, issued_qty_lpl,
                        strength);
                }
