...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "Total Seizures  - Last 7 Days"); // caption will change dynamically based on chart label
        chartConfig.Add("xAxisName", "Date"); // xaxis name will chnage dynamically based on chart label
        ...
        chartConfig.Add("valueFontColor", "#114b5f ");
        chartConfig.Add("valueFontBold", "1");
        chartConfig.Add("palettecolors", "#e26d5c");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT ltrim(rtrim(date(sb.creation_date))) as creation_date ,count(seizureno) as SeizureNoCount FROM exciseautomation.seizure_basicinfo sb  inner join exciseautomation.district_master dm on dm.district_code =sb.district_code"
                + " where sb.creation_date between '" + DateTime.Now.AddDays(-6).ToString("dd/MM/yyy") + "' and '"+ DateTime.Now.ToString("dd/MM/yyy")+"' group by sb.creation_date order by sb.creation_date asc", cn))
            {
                // fill data table
                da.Fill(dt);
