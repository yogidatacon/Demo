...
public class Handler : IHttpHandler
{
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT  propertystatus ,count(seizure_propertydetails_id) as PropertyCount"
                    + " from exciseautomation.seizure_propertydetails    sv"
                    + " group by  propertystatus", cn))
            {
                // fill data table
                da.Fill(dt);
