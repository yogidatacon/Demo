...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "Seized Vehicle Status"); // caption will change dynamically based on chart label
        chartConfig.Add("xAxisName", "Vehicle Status"); // xaxis name will chnage dynamically based on chart label
        chartConfig.Add("yAxisName", "Total Vehicles Seized");
        ...
        chartConfig.Add("palettecolors", "#29c3be");
        chartConfig.Add("formatNumberScale", "0");
        chartConfig.Add("plotBinSize", "3");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT  vehiclestatus ,count(seizure_vehicledetails_id) as VehicleCount from exciseautomation.seizure_vehicledetails sv"
                    + " group by  vehiclestatus order by count(seizure_vehicledetails_id) desc", cn))
            {
                // fill data table
                da.Fill(dt);
