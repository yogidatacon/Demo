...
public class Handler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        //string query = "";
        //string label = "";
        //if (string.IsNullOrEmpty(drillLevel))m
        //{
        //    drillLevel = "0";
        //    // build custom query 
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "Excisable Article's Seized"); // caption will change dynamically based on chart label
        chartConfig.Add("xAxisName", "Articles"); // xaxis name will chnage dynamically based on chart label
        chartConfig.Add("yAxisName", "Qauntity");

        chartConfig.Add("theme", "fusion");
        chartConfig.Add("defaultcenterlabel", "Article Status");
        ...
        chartConfig.Add("palettecolors", "#008080");
        chartConfig.Add("formatNumberScale", "0");
        chartConfig.Add("baseFontSize", "12");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select case"
                                                                + " when articlecategory ='IMFL' then 'IMFL(in Ltrs)' "
                                                                + " when articlecategory ='Country Liquor' then 'Country Liquor(in Ltrs)'"
                                                                + " when articlecategory ='ID Liquor'  then 'ID Liquor(in Ltrs)'"
                                                                + " when articlecategory ='Java Mahua' then 'Java Mahua(in Kgs)'"
                                                                + " when articlecategory ='Toddy' then 'Toddy(in Ltrs)'"
                                                                + " when articlecategory ='Mahua' then 'Mahua(in Kgs)'"
                                                                + " when articlecategory ='Spirit' then 'Spirit(in Ltrs)'"
                                                                + " when articlecategory ='Bhaang' then 'Bhaang(in Ltrs)'"
                                                                + " when articlecategory ='Ganja' then 'Ganja(in Kgs)'"
                                                                + " else articlecategory end as Articlecategory,TotalQuantity  from exciseautomation.Articles_Quanity_View", cn))
            {
                // fill data table
                da.Fill(dt);
