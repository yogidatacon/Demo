...
public class Handler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string[] levelValueMapping = new string[] {"district", "createddate","thana"};
    ...
    {
        string query;
        // query = "select TO_CHAR(raid_date :: date, 'dd/mm/yyyy') as " + columnName + ", count(raid_date) as SeizureNoCount from exciseautomation.seizure_basicinfo group by " + columnName;
        query = "select TO_CHAR(raid_date :: date, 'dd/mm/yyyy') as createddate, count(raid_date) as SeizureNoCount from exciseautomation.seizure_basicinfo a inner join exciseautomation.district_master c on a.district_code=c.district_code Group by createddate";
        return query;
    }
    private string BuildQuery(string columnName, string parentValue, string parentName)
    {
        string query;
        //query = "select TO_CHAR(raid_date :: date, 'dd/mm/yyyy') as " + columnName + ", count(raid_date) as SeizureNoCount from exciseautomation.seizure_basicinfo where " + parentName + "= '" + parentValue + "'Group by " + columnName;
        query = "select TO_CHAR(raid_date :: date, 'dd/mm/yyyy') as createddate, count(raid_date) as SeizureNoCount from exciseautomation.seizure_basicinfo a inner join exciseautomation.district_master c on a.district_code=c.district_code where c.district_name= '" + parentValue + "' Group by createddate";
        return query;

    }
    private string ProcessChartData(DataTable dt,string parentValue, string drillLevel, int maxLevel)
    {
        ...
        string linkParam = "newchart-jsonurl-Handler/DistrictHandler.ashx?label={0}&drillLevel={1}";
        int i = 0;
        ...
        chartConfig.Add("caption", "Total Seizures by District- Till Date (  "+i.ToString()+" )"); // caption will change dynamically based on chart label
        chartConfig.Add("xAxisName",  "District" ); // xaxis name will chnage dynamically based on chart label
        chartConfig.Add("yAxisName", "Total Locks CreatedDate");
        chartConfig.Add("labeldisplay", "ROTATE");
