...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "Molasses Status "); // caption will change dynamically based on chart label
        chartConfig.Add("xAxisName", "Production v/s Indent v/s Allotted"); // xaxis name will chnage dynamically based on chart label
        chartConfig.Add("yAxisName", "Quantity");
        ...
        chartConfig.Add("paletteColors", "#08B9F7,#4CBFA0,#F2BC50,#C569C6");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select 'Probable Molasses Production',sum(mf2_tobe_alloted_total) as totalproduction from exciseautomation.mf2 union all"
                                                                + " select 'Total Indent Received For Molasses',sum(mf1_qty_req_total) as totalindented from exciseautomation.mf_1 union all"
                                                                + " select 'Total Provisional Molasses Allotted',sum(a_qty) as totalAllotted from exciseautomation.mf_allotment where allotment_status='I' union all"
                                                                + " select 'Total Balance Allotment',aa-bb from  (select cast(sum(mf1_qty_req_total) as float) as aa    from exciseautomation.mf_1) as a , (select cast(sum(a_qty) as float) as bb  from exciseautomation.mf_allotment where allotment_status='I') as b ", cn))
            {
                // fill data table
                da.Fill(dt);
