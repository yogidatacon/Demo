...
public class Handler : IHttpHandler
{
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select 'Bail Request' as  total_bail_request,total_bail_request from exciseautomation.casemgmt_chart_accused_dashboard where district='"+userid+"' union all"
                                                                + " select  'Bail Granted' as  bail_granted,bail_granted from exciseautomation.casemgmt_chart_accused_dashboard where district='" + userid + "' union all"
                                                                + " select 'Bail Rejected' as  total_bail_rejected,total_bail_rejected from exciseautomation.casemgmt_chart_accused_dashboard where district='" + userid + "' union all"
                                                                + " select  'Arrested' as  total_accused_absconding,total_accused_absconding from exciseautomation.casemgmt_chart_accused_dashboard  where district='" + userid + "'", cn))
            {
                // fill data table
                da.Fill(dt);
