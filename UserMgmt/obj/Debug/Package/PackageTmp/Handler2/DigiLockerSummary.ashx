...
public class Handler : IHttpHandler,System.Web.SessionState.IRequiresSessionState
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        string linkParam = "P-detailsWin,width=400,height=300,toolbar=no,scrollbars=yes, resizable=no-http://fusioncharts.com?label={0}&drillLevel={1}";
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("select * from exciseautomation.View_digilocker_overallstatus",cn))
            {
                // fill data table
                da.Fill(dt);
