...
public class Handler : IHttpHandler
{
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT cs.status,count(vehicleid) as VehicleCount "
                    + " from exciseautomation.casemgmt_seizedvehicledetails right join exciseautomation.casemgmt_status cs"
                    + " on cs.status=vehiclestatus group by cs.status", cn))
            {
                // fill data table
                da.Fill(dt);
