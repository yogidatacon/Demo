...
public class Handler : IHttpHandler,System.Web.SessionState.IRequiresSessionState
{
   ...
   private string BuildQuery(string columnName)
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "Total Seizures by District "); // caption will change dynamically based on chart label
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT district,count(seizureno) as SeizureNoCount FROM"
                + " exciseautomation.casemgmt_basicinformation where district='" + userid.ToString() + "' group by district ", cn))
            {
                // fill data table
                da.Fill(dt);
