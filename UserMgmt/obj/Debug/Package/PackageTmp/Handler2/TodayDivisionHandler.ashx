   ...
   private string BuildQuery(string columnName)
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT count(seizureno) as SeizureNoCount FROM"
                + " exciseautomation.casemgmt_basicinformation", cn))
            {
                // fill data table
                da.Fill(dtt);
        ...
        chartConfig.Add("caption", "Total Seizures by Division Today"); // caption will change dynamically based on chart label
        ...
        chartConfig.Add("defaultcenterlabel", "Total Today " + i);
        ...
        chartConfig.Add("paletteColors", "#D7BDE2,#AED6F1,#A3E4D7,#F9E79F,#FAD7A0,#EC7063,#F4D03F,#E9967A,#7DCEA0");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            con.Open();
            using (NpgsqlCommand command = new NpgsqlCommand())
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT divison,count(seizureno) as SeizureNoCount FROM"
                + " exciseautomation.casemgmt_basicinformation where CreatedDate='" + DateTime.Now.ToString("dd-MM-yyyy") + "' group by divison", cn))
            {
                // fill data table
                da.Fill(dt);
