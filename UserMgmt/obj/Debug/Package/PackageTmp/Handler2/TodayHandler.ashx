...
public class Handler : IHttpHandler
{
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("xAxisName", "Division"); // xaxis name will chnage dynamically based on chart label
        ...
        chartConfig.Add("palettecolors", "#82c7f0");
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            ...
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT divison,count(seizureno) as SeizureNoCount FROM exciseautomation.casemgmt_basicinformation where CreatedDate='" + DateTime.Now.ToString("dd-MM-yyyy") + "' group by divison", cn))
            {
                // fill data table
                da.Fill(dt);
