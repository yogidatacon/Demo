...
public class Handler : IHttpHandler
{
   ...
   private string BuildQuery(string columnName)
    ...
    private string ProcessChartData(DataTable dt)
    {
        ...
        chartConfig.Add("caption", "Total Seizures by Thana  Today ( " + i+" )"); // caption will change dynamically based on chart label
    ...
    private void GetChartData(ref DataTable dt)
    {
        ...
        using (NpgsqlConnection con = new NpgsqlConnection(connstring))
        {
            con.Open();
            using (NpgsqlCommand command = new NpgsqlCommand())
            using (NpgsqlDataAdapter da = new NpgsqlDataAdapter("SELECT thana,count(seizureno) as SeizureNoCount"
                + " FROM exciseautomation.casemgmt_basicinformation where district='" + userid.ToString() + "'"
                + " and CreatedDate='" + DateTime.Now.ToString("dd-MM-yyyy") + "' group by thana order by count(seizureno) desc", cn))
            {
                // fill data table
