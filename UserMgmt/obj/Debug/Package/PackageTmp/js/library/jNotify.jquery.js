...
(function($){
	...
	$.jNotify = {
		...
		_show:function(msg) {
			...
			switch (opts.VerticalPosition) {
				case 'top':
					...
					if (opts.autoHide)			
						$.jNotify._close($Div[0].id, true);
					else
						$Div.css('cursor','pointer').click(function(e){
							$.jNotify._close(e, false);
						});
					break;
				case 'center':
					$Div.animate({
		...
		_close: function (e, id) {
			var CloseDiv = ArrayDiv;
			for (var i = 0; i < ArrayDiv.length; i++) {
				if (id) {
					if (e == ArrayDiv[i][0].id) {
						CloseDiv = ArrayDiv[i];
						ArrayDiv.splice(i, 1);
						break;
					}
				} else {
					if (e.currentTarget.id == ArrayDiv[i][0].id) {
						CloseDiv = ArrayDiv[i];
						ArrayDiv.splice(i, 1);
						break;
					}
				...
				switch (opts.VerticalPosition) {
					case 'top':
						if(!opts.autoHide)
							opts.TimeShown = 0;
						CloseDiv.stop(true, true).delay(opts.TimeShown).animate({ // Tanks to Guillaume M.
							top: posTop-opts.LongTrip,
							opacity:0
